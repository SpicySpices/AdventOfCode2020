...
let times = 0;
for(let list of str){
    let stuff = list.split(':');
    stuff[0] = stuff[0].split(' ');
    stuff[0][0] = stuff[0][0].split('-');
    let word = stuff[1].trim();

    let wanted = stuff[0][1];
    let min = stuff[0][0][0];
    let max = stuff[0][0][1];
    let num = 0;

    let good = word[min-1] == wanted ^ word[max-1] == wanted;
    if(word[min-1] == wanted ^ word[max-1] == wanted){
        times++;
    }
}
